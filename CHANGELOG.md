...
- **Bundled Noto symbol fonts** — `Noto Sans Symbols` + `Noto Sans Symbols 2` (OFL) are bundled under `assets/fonts/` and passed to the PDF compile, so astrology glyphs render from packaged fonts rather than relying on the host's fonts (the system "Noto Sans Symbols 2" is missing the classic planets ☿♀♃♄) — portable to CI and other machines.

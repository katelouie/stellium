...
- **Translation coverage checker** — New `web/i18n/check_coverage.py` extracts every `_("…")` translator call from `web/` via the AST and fails if any string lacks a translation in a locale file (and lists orphaned keys). Wired into the CI workflow on the canonical platform so missing translations break the build instead of silently falling back to English.

...
- **Reports and charts render in another language.** `ReportBuilder().with_locale("zh_CN")`
  ...
  aspects, house systems, dates. The wheel body is glyphs and stays language-neutral; only
  its header/info text localizes. Dates are laid out per the locale, not word-swapped, so a
  Chinese chart reads `1879年3月14日`, not a reordered English string. English output is
  unchanged.
...
- **A public i18n surface** — `t()`, `term()`, `msg()`, `render()`, `format_date()`,
  `format_time()`, `available_locales_info()`, and a translator template
...
- **Essential-dignity names are capitalized** in the `show_details` view — "Peregrine,
  ...
  the engine's internal form leaking into display).

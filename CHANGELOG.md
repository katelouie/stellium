...
- **A public i18n surface** — `t()`, `term()`, `msg()`, `render()`, `format_date()`,
  ...
  The translatable vocabulary now includes `direction` (N/S/E/W, reordered per locale via
  the `format.latitude` / `format.longitude` patterns) and `polarity` (Yang/Yin/Balanced).

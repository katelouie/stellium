...
- **PDF rendering is now data-driven** — `ReportBuilder`'s PDF path serialises the report (chart + section data) to a JSON contract that the bundled Typst design system renders, replacing the previous renderer that built Typst markup as Python strings. Visual concerns (colour, type, layout) live entirely in the `.typ` design system; the Python side only supplies structured data and the chosen theme. The report wheel is drawn with its header/info-box stripped and a transparent background, themed to match the PDF, so it composites cleanly onto the page.
...
- **PDF reports — a new Typst design system with five themes** — `ReportBuilder().render(format="pdf", theme=…)` now produces an editorial, magazine-quality PDF built on a bundled Typst design system (`stellium/presentation/typst_theme/`) instead of the old inline string-built Typst. One shared architecture (page frame, glyph system, section components) drives five interchangeable themes selected by name: `"house"` (default — warm cream + aubergine + gold), `"sepia"` (antique book-plate), `"celestial"` (dark gold-on-indigo), `"blues"` (dark cyanotype blueprint, with a graph-paper page background), and `"greyscale"` (laser-printer-safe: outlined discs/badges, no ink fills, grey wheel). A theme is pure data — eight colour tokens + a display/body/mono font trio + a sign-tint palette + a `laser` flag — so adding one is a palette entry, not code. Section vocabulary: a title page with the embedded wheel + Sun/Moon/Ascendant cards, a distribution "Snapshot" (element/modality bars + polarity), planet positions with sign-tinted glyph discs and house badges, a native aspectarian matrix + aspect list, a native dispositor graph, moon phase, and themed generic tables (which double-column and fill the page for narrow/long data). Aspect colours, zebra shades, and gridlines are keyed to each theme to match the chart wheel. Requires the `typst` binary.
- **Native, portable dispositor graph** — the dispositor graph is now drawn natively (Typst primitives in the PDF; svgwrite for HTML) from a shared layered layout, replacing graphviz. `stellium.engines.dispositor_graph_data(result)` lays out the functional dispositor DAG (rank = BFS distance from the sink) into positioned nodes + edges; `render_dispositor_svg(graphs, filename=None)` renders them to an SVG string (themed panels, gold final-dispositor nodes, bidirectional mutual-reception arrows). Both are glyph-correct and need no graphviz. `from stellium.engines import dispositor_graph_data, render_dispositor_svg`.
- **Standalone moon-phase illustration** — `stellium.visualization.moon_phase_svg(chart, filename=None, size=200, style=None, background=None, show_label=False)` renders just a chart's Moon phase as its own square SVG (transparent by default), for embedding a phase illustration on its own. `draw_moon_phase_standalone()` gains `background` and `show_label` options. `from stellium.visualization import moon_phase_svg, draw_moon_phase_standalone`.
- **`ChartDrawBuilder.without_chart_info()` and `.with_transparent_background()`** — new wheel-drawing options: suppress the in-wheel info box (symmetric with `without_header()` / `without_moon_phase()`), and skip the full-canvas background rectangle so the wheel composites onto whatever embeds it (used by the themed report wheels).
- **Structured section payloads** — `PlanetPositionSection` and `AspectSection` now emit structured data (planet fields; aspectarian bodies + aspect pairs with orb/applying) alongside their string `rows`, so structure-aware renderers (the Typst PDF) can draw discs, badges, and matrices while the terminal/HTML/markdown renderers keep consuming the derived rows unchanged.
- **Bundled fonts — the PDF is now fully self-contained** — the complete font stack every theme needs (the display/body/mono faces: Cinzel, Cormorant Garamond, EB Garamond, Newsreader, Spectral, Space Grotesk, IBM Plex Serif/Mono; plus the `Noto Sans Symbols` + `Noto Sans Symbols 2` glyph fonts) now ships **inside the package** at `stellium/data/fonts/` (18 deduplicated files — variable fonts where available, statics only for static-only families) and is wired into the wheel via `package-data`. `_font_paths()` resolves this package-relative location, so a plain `pip install stellium` renders identical PDFs on any machine with no host-font dependency (verified with `ignore_system_fonts=True` across all five themes). This also fixes the classic-planet tofu from the host's "Noto Sans Symbols 2" missing ☿♀♃♄.
- **Sect rectification — the compare-hypothesis workbench** (`stellium.rectification`) — an honest, human-in-the-loop **sect** (day/night) analysis for charts whose birth *time* is uncertain. It does not claim to invert time (an empirical study found minute-level rectification is an ill-posed inverse); it recovers the one recoverable bit — sect — at ~70% (LOO-validated, 70.6% out-of-sample) and lays out both hypotheses for adjudication. `analyze_sect(chart, events=None, temperament=None) -> SectAnalysis` gives the daylight prior, a calibrated `p_day` (a two-feature logistic frozen from the 63-chart corpus fit), both day/night structures (sect light, out-of-sect malefic, in-sect benefic with dignity), a Moon-band flag, event/temperament evidence, and firdaria convergence — all flagged *indicator, not oracle*. `convergence_matrix(chart)` adds an exploratory two-lens Tebbs display (structural band × solar-arc/transits/profection, plus an event-hook histogram; whisper-level, never summed). Report sections `ReportBuilder().with_sect_rectification()` and `.with_sect_convergence_matrix()` render it in every format. Events/temperament auto-resolve for notables via the biography API. `from stellium import analyze_sect, SectAnalysis, convergence_matrix`.
...
- **`docs/methodology/` — traditional-method sourcing** — a new documentation area recording *what Stellium's Hellenistic/Perso-Arabic techniques implement and on whose authority*, separate from the practitioner guide (`docs/astrology/`) and the code reference (`docs/development/`). `methodology/README.md` is the curated "Traditional Methods & Sources" (per-technique defaults, citations, contested forks, and simplifications); `methodology/research/` holds the three source-verification reports (planetary years, ZR, Firdaria) as versioned provenance — brought in-repo (they had only lived in `~/Downloads`, which the specs pointed at) and polished into pure scholarship. Also: `examples/hellenistic_cookbook.py`, a 16-example tour of the new techniques.
- **Length of life (hyleg / alcocoden / years-table, Lilly)** — `chart.length_of_life()` returns a transparent, itemized `LengthOfLifeResult`: the chosen hyleg (with a candidacy trace), the alcocoden and its angularity, the base years (angular → greater, succedent → mean, cadent → least, from `planetary_years`), and every modifier itemized (benefics/lights add, malefics subtract, fall/retrograde halves, combust → months) summing into the total. Framed explicitly as a *computed traditional indicator, not a lifespan prediction*. `method="ptolemy"` (the directional method) is reserved but not implemented. Two reusable primitives ship alongside: `chart.hyleg()` and `almuten_of_degree(longitude, sect)` (the essential-dignity victor over a point — distinct from `get_strongest_planet`). Public API on `stellium` / `stellium.engines`.
- **Almuten of a degree** — `stellium.engines.almuten_of_degree` scores each planet's essential dignity at a longitude (domicile 5, exaltation 4, triplicity 3, term 2, face 1) and returns the victor with its per-planet scores and ties; sect-sensitive triplicity, node exaltations never scored.
- **Firdaria (Persian time-lord system)** — `chart.firdaria()` returns a `FirdariaTimeline` of major periods and seven Chaldean sub-periods, sect-ordered (day: Sun-first, night: Moon-first). Presets cover the one genuine scholarly fork — where the lunar nodes fall in *nocturnal* charts: `abu_mashar` (default; nodes at the end in both sects), `bonatti` (nocturnal nodes after Mars), `al_biruni` (alias), and `no_nodes` (seven-planet, 70-year cycle). Real 365.2425-day year, repeats past 75, refuses on unknown-time charts. Public API: `from stellium import FirdariaTimeline, FirdariaPeriod` / `from stellium.engines import FirdariaEngine`.
- **Canonical planetary-years primitive (`stellium.core.planetary_years`)** — the least/greater/mean/greatest year families + Firdaria periods, hoisted out of Zodiacal Releasing into one source-attributed module consumed by ZR, Firdaria, and future length-of-life/decennials work. Mean years are derived `(least + greater)/2`; greatest years are stored as *attributed variants* (De Vore vs. astronomical; the contested Moon at 25/320/420/520) with no single default. A test derives the non-luminary greater years from the Egyptian term data and asserts they match (360), guarding transcription drift.
- **Structured diagnostics: `warnings` + `logging` instead of `print`** — Library code no longer writes to stdout uninvited. New public surface: `stellium.configure_logging(level=...)` (one-call setup over a silent-by-default `stellium` logger with a `NullHandler`), and a warning hierarchy `StelliumWarning` ⊃ `DataQualityWarning`, `GeocodingWarning`, `ConfigurationWarning`, `MissingEphemerisWarning` — filter the base class to silence everything (`warnings.filterwarnings("ignore", category=StelliumWarning)`). Caller-facing conditions (skipped import rows, geocoding failures, invalid orb/house config) now emit typed, filterable warnings; internal operational detail (cache/file IO) logs through the `stellium` logger; the CLI writes via `click.echo`. A `ruff` `T20` lint guard bans new bare `print()` in the package. See [`docs/development/specs/STRUCTURED_LOGGING_SPEC.md`](docs/development/specs/STRUCTURED_LOGGING_SPEC.md).
...
- **Zodiacal Releasing toggles** — `ZodiacalReleasingEngine` and `ZodiacalReleasingAnalyzer` now accept `year_length` (360 default | 365.25), `capricorn_years` (27 default | 30), and `loosing_target` (`"opposite"` default | `"trine"`, the jump destination for the loosing of the bond). The engine's dates are validated end-to-end against Brennan's published George W. Bush example (Spirit-release Leo peak, 10th from Fortune, beginning 1998).
- **Notable source-verification audit (218 entries)** — Applied a provenance audit to the notables database: corrected fabricated Rodden ratings (many "AA" inherited from Astro-Seek mirrors were really DD/X/C — Fidel Castro, Hugo Chávez, Leslie Cheung, …), swapped `"Stellium Test Suite"` placeholder sources for real citations (Einstein's birth certificate → C→AA), fixed birthplaces (Aung San Suu Kyi born Rangoon not Delhi; Eva Perón), corrected event times (Moon Landing was ~5h off; Titanic; Pearl Harbor; 1929 crash), and deduplicated Leonardo da Vinci and Mozart. A follow-up provenance pass resolved the manual-verification queue against primary sources: more aggregator-"AA" collapses (Franz Kafka → DD, Aishwarya Rai → C, Faye Wong / Amrita Pritam → X with their fabricated times removed), two document-backed upgrades (Catherine the Great → A on her father's cited 1729 letter; Prince Philip confirmed AA / 10:00 at Mon Repos, Corfu on the 2021-discovered birth certificate, with the villa coordinates fixed), Steve Wozniak's round-hour corrected to the real 09:45, and Mohamed Bouazizi's self-immolation time corrected (16:00 → ~11:30).
- **`Notable.has_reliable_time` and `Notable.verification_notes`** — New fields carrying the audit's routing flag and justification. `has_reliable_time` distinguishes a genuine recorded birth time from a noon/midnight default, rectification, or fabrication.
- **`CalculatedChart.is_time_unknown` on the base class** — The `is_time_unknown` predicate is now defined on `CalculatedChart` (returns `False`), not only on `UnknownTimeChart`, so it is safe to call on any chart without an `isinstance` check.

### Changed

- **Removed `stellium.utils.cache_utils`** *(breaking)* — The print-based cache display module (`print_cache_info`, `clear_ephemeris_cache`, `clear_geocoding_cache`) was redundant: the `stellium cache` CLI command already provides the same via `click.echo`, and nothing else used it. `clear_cache` and `cache_size` remain importable from `stellium.utils` (they always lived in `stellium.utils.cache`). Use the `stellium cache` CLI for cache display.
...
- **Notables with no reliable birth time now build as unknown-time charts** *(behavior change)* — `ChartBuilder.from_notable(...)` routes to an `UnknownTimeChart` when the notable's `has_reliable_time` is `False` (a noon/midnight default, rectification, or fabrication) or when no clock time is recorded, instead of silently charting a fake time. Reliable-time notables are unaffected. Branch on `has_reliable_time`, not `data_quality` (a source rating is not a time-validity claim).
- **`from_notable("Mozart")` removed — use `"Wolfgang Mozart"`** *(breaking)* — The audit's structural dedup deleted the duplicate bare `"Mozart"` entry in favour of the canonical `"Wolfgang Mozart"`, so `from_notable("Mozart")` now raises `ValueError: No notable found`. (Leonardo da Vinci was likewise deduplicated, but its canonical name is unchanged.) Bundled docs and example cookbooks were updated to the canonical name.
- **Unknown-time charts now share the main calculation pipeline** — `.with_unknown_time()` charts were built by a separate code path that had drifted from the main one, silently omitting declination aspects, analyzers (aspect patterns, zodiacal releasing), the component manifest, injected metadata, and — for sidereal charts — the zodiac configuration itself. An `UnknownTimeChart` now runs the identical pipeline as a normal chart, differing only in that it skips houses/angles and carries a Moon day-range. This removes a whole class of "known-time feature silently missing on no-time charts" bugs (see Fixed).
- **Single noon-normalizer** — The date→noon normalization for unknown-time charts existed as two separate implementations (in `Native` and in `ChartBuilder.with_unknown_time()`); both now route through one shared `build_chart_datetime()` helper so they cannot drift.

### Fixed

- **Sagittarius Egyptian terms were transposed** *(dignity output change)* — The last two Egyptian bounds of Sagittarius were swapped: Mars held 21–26° and Saturn 26–30°, but the canonical Ptolemaic terms (Tetrabiblos I.20; Lilly; Houlding) give Saturn 21–26° and Mars 26–30°. A planet at 21–26° Sagittarius therefore reported Egyptian-term ruler **Mars** instead of **Saturn**, affecting `get_planet_dignity`, term scores, and downstream weighting. Now corrected; the error was surfaced (and is now permanently guarded) by the planetary-years term-sum cross-check.
...
- **Synastry/multi-chart crashed when a chart had no birth time** — Building a `MultiChart` with a timeless chart (e.g. a notable whose source doesn't support a reliable time) raised `AttributeError: 'NoneType' object has no attribute 'cusps'` during house-overlay calculation, and `ValueError: No house systems calculated` when drawing the biwheel. Three fixes: (1) house-overlay calculation now skips the timeless chart as a *house owner* — cross-chart aspects (which never need houses) and overlays *into* the timed chart are still computed; (2) biwheel/multiwheel rendering skips the house-cusp and angle rings for any timeless chart's wheel; (3) the builder's internal-aspect step now uses `dataclasses.replace`, so a chart's concrete type (`UnknownTimeChart`) and all fields survive — previously it was silently downgraded to a plain chart with `is_time_unknown=False`, which is what defeated the rendering guards. Astrologically this matches practice: two people can be compared by aspect even when one birth time is unknown; only the timeless partner's house overlays are unavailable.
...
- **Registry crashed on timeless notables** — the loader read `entry["hour"]` by direct access, so a notable with no recorded time (e.g. Alfred Hitchcock, after the audit removed his speculative 03:15) raised `KeyError` and was silently dropped from the registry. `hour`/`minute` are now optional and such notables load and route to unknown-time.
- **Venus miscoded as "fall" in Gemini under modern dignities (also Sagittarius)** — The modern essential-dignity blocks for Gemini and Sagittarius had their same-ruler partner sign's exaltation/fall grafted in: Gemini held Virgo's (Mercury exalt / Venus fall) and Sagittarius held Pisces's (Venus exalt / Ceres fall), producing bogus dignities and scores (a Venus in Gemini scored a false −4 "fall"). Both now mirror their traditional node-based exaltation/fall. Affects `get_planet_dignity`, dignity scores, `get_strongest_planet`, and any downstream weighting.
...
- **Declination aspects dropped on unknown-time charts** — `.with_declination_aspects()` was silently ignored when `.with_unknown_time()` was set, even though declinations depend only on the date (not the birth time) and are among the most useful data an unknown-time chart can offer. Parallels and contraparallels now calculate on unknown-time charts.
- **Empty "Essential Dignities" in `to_prompt_text()`** — The dignities section printed its header but no per-planet rows, because the renderer read a singular `"dignity"` key that the dignity component never emits (it produces a `"dignities"` list). Planet rows now render, with dignity names and scores.
- **Sidereal (and other config) ignored on unknown-time charts** — A `.with_sidereal(...)` + `.with_unknown_time()` chart computed tropical positions and dropped its ayanamsa/zodiac metadata, because the unknown-time path called the ephemeris without the `CalculationConfig`. Sidereal and all other configuration now apply to unknown-time charts.
- **Analyzers, component manifest, and injected metadata missing on unknown-time charts** — `.add_analyzer(...)` results (aspect patterns, zodiacal releasing), the component manifest behind `available_components()`, and externally injected metadata (e.g. from `ReturnBuilder`) were all skipped on unknown-time charts. They now populate as they do for known-time charts.
...
- **BaZi Day Master Strength Analysis (日主强弱)** — `analyze_strength(chart)` returns a `StrengthAnalysis` with classification (Very Strong/Strong/Moderate/Weak/Very Weak), weighted score, component breakdown (seasonal strength, root count, support vs drain), favorable/unfavorable elements, and display/export methods. Uses traditional seasonal tables with Wu Xing cycle scoring. New `Element.produced_by` and `Element.controlled_by` reverse-cycle properties added. Also available as `bazi.strength()` method on the chart.
...
- **Chart shape span failure** at 0°/360° seam: Bundle/Bowl shapes straddling the Aries point were misidentified due to inflated span calculation. (GitHub issue #24)
